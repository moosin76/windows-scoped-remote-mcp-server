...
describe("ProviderScheduler", () => {
  it("rediscovers an unavailable provider and refreshes its tools", async () => {
    ...
    const changed = vi.fn();
    const scheduler = new ProviderScheduler(registry, {
    ...
    expect(registry.listCachedTools().map((tool) => tool.tool.name)).toEqual([
      "godot_get_scene",
    ]);
  ...
  it("detects a remote tool list change", async () => {
    ...
    provider.tools = [
      ...provider.tools,
      { name: "run_project", inputSchema: { type: "object" } },
    ];
    await scheduler.pollNow();
