...
export function createMcpServer(
  ...
  const server = new McpServer(
    ...
    {
      instructions:
        "This server is a Windows remote development environment with multi-root workspace support. Tools operate on the host. Use list_workspaces, get_active_workspace, and switch_workspace to inspect and switch projects; use exec_command for PowerShell / CMD commands, builds, package management, and git; run_script for complete PowerShell, Batch, Node.js, or Python scripts; and file tools (list_directory, read_file, write_file, replace_in_file, etc.) for direct file operations inside the active workspace.",
      capabilities: {
        tools: {},
        prompts: {},
        logging: {},
