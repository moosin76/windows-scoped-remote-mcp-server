...
class PersistentOAuthStore implements OAuthRegisteredClientsStore {
  ...
  async getClient(
    ...
    if (!client || clientMetadataProblem(client)) {
      // Auto-register fallback client for ChatGPT connector
      client = {
        client_id: clientId,
        ...
        redirect_uris: [
          ...
          "https://chat.openai.com/aip/g-dev/oauth/callback",
        ],
  ...
  async registerClient(
    ...
    const registered: OAuthClientInformationFull = {
      ...
      client_id_issued_at:
        supplied.client_id_issued_at || Math.floor(Date.now() / 1000),
    };
  ...
  private revokeGrantWithoutPersist(
    grantId: string,
    preserveReplayEvidence = false,
  ): void {
    for (const [hash, token] of Object.entries(this.state.tokens)) {
      if (
  ...
  async rotateRefreshToken(
    ...
    return this.mutate(() => {
      ...
      return {
        status: "ok",
        tokens: this.issueTokenPairWithoutPersist(
          clientId,
          scopes,
          resource,
          grantId,
  ...
  async getOrCreateAutoRecoveredToken(
    ...
    return this.mutate(() => {
      const clientId =
        Object.keys(this.state.clients)[0] ||
        "944bf9d931091fe77acf86b37677bc41";
...
function renderAuthorizationPage(
  ...
  const clientName = client.client_name || "ChatGPT MCP 클라이언트";
...
<head>
  ...
  <title>Windows Scoped Remote MCP 승인</title>
  <style>
    ...
    p { color: #b8c1d8; line-height: 1.55; font-size: 14px; }
    .warning { padding: 12px; border-radius: 10px; background: #3c2316; color: #ffd8bd; font-size: 13px; margin: 16px 0; }
    .error { color: #ff9f9f; font-weight: 650; margin: 10px 0; font-size: 14px; }
    label { display: block; margin: 20px 0 8px; font-weight: 650; font-size: 14px; }
    ...
    button { width: 100%; margin-top: 16px; padding: 12px; border: 0; border-radius: 9px; background: #0284c7; color: white; font: inherit; font-weight: 700; cursor: pointer; transition: background 0.2s; }
    button:hover { background: #0369a1; }
    small { display: block; margin-top: 14px; color: #8390aa; overflow-wrap: anywhere; font-size: 12px; }
  </style>
</head>
<body>
  <main>
    <h1>MCP 서버 연결 승인</h1>
    <p><strong>${escapeHtml(clientName)}</strong>에서 사용자의 Windows Scoped MCP 서버 도구 사용 권한을 요청했습니다.</p>
    <p class="warning">⚠️ 승인하면 연결된 MCP 클라이언트가 이 컴퓨터의 격리된 프로젝트 폴더(d:\\Godot\\localRemoteMcp) 내에서 파일과 명령어를 실행할 수 있습니다.</p>
    ${invalidKey ? '<p class="error">❌ 인증키가 올바르지 않습니다.</p>' : ""}
    <form method="post" action="/authorize" autocomplete="off">
      ${fields}
      <label for="access_key">MCP 인증키 (MCP_AUTH_TOKEN)</label>
      <input id="access_key" name="access_key" type="password" required autofocus autocomplete="current-password" placeholder="비밀번호 토큰 입력...">
...
export class RemoteDevOAuthProvider implements OAuthServerProvider {
  ...
  constructor(private readonly config: AppConfig) {
    const pubUrl = config.publicUrl || `http://localhost:${config.port}`;
    this.issuerUrl = new URL(config.oauthIssuerUrl || pubUrl);
    this.resourceUrl = new URL(
      config.oauthResourceUrl || `${pubUrl.replace(/\/$/, "")}/mcp`,
    );
  ...
  private validateScopes(scopes: string[] | undefined): string[] {
    const requested =
      scopes && scopes.length > 0 ? [...new Set(scopes)] : [...OAUTH_SCOPES];
    if (
  ...
  async authorize(
    ...
    if (
      !(client.grant_types ?? ["authorization_code"]).includes(
        "authorization_code",
      )
    ) {
      throw new UnauthorizedClientError(
        "Client is not authorized for authorization_code",
      );
    }
    ...
    const approvalKey = (
      this.config.oauthApprovalKey || this.config.authToken
    )?.trim();

    if (!accessKey || !approvalKey || !tokensEqual(accessKey, approvalKey)) {
      response
        ...
        .send(
          renderAuthorizationPage(
            client,
            { ...params, scopes, resource: new URL(resource) },
            Boolean(accessKey),
          ),
        );
      return;
    ...
    this.authorizationCodes.set(code, {
      ...
      scopes,
      expiresAt:
        Date.now() + this.config.oauthAuthorizationCodeTtlSeconds * 1000,
    });
  ...
  async exchangeAuthorizationCode(
    ...
    if (resource && record.resource !== this.validateResource(resource)) {
      throw new InvalidGrantError("Invalid authorization code binding");
    }
    ...
    try {
      ...
      if (
        ...
        !this.authorizationCodes.has(authorizationCode)
      ) {
  ...
  async exchangeRefreshToken(
    ...
    if (!client.grant_types?.includes("refresh_token")) {
      throw new UnauthorizedClientError(
        "Client is not authorized for refresh_token",
      );
    }
  ...
  async verifyAccessToken(token: string): Promise<AuthInfo> {
    ...
    if (this.config.authToken && tokensEqual(token, this.config.authToken)) {
      return {
        token,
