<template>
  <q-page class="extension-page">
    <section class="hero section-shell">
      ...
      <p>{{ tr('hero') }}</p><div class="ai-callout"><strong>{{ tr('aiCalloutTitle') }}</strong><p>{{ tr('aiCalloutText') }}</p><div class="flow-line"><span>① MCP 서버 실행</span><b>→</b><span>② AI에게 연결 요청</span><b>→</b><span>③ WSR 재시작</span><b>→</b><span>④ ChatGPT 새로고침</span></div></div>
    </section>
  ...
  url: <b>"http://127.0.0.1:8000/mcp"</b>,
    ...
    <section class="section-shell lifecycle">
      ...
      <div class="scheduler"><div><b>ProviderScheduler</b><span>health check · reconnect · tools/list</span></div><div><b>10s</b><span>{{ tr('connected') }}</span></div><div><b>5s</b><span>{{ tr('retry') }}</span></div></div>
    </section>
...
const d={ko:{hero:'WSR은 특정 MCP에 종속되지 않습니다. Streamable HTTP를 지원하는 Remote MCP를 Provider로 연결하고, namespace를 통해 하나의 Gateway 안에서 안전하게 확장할 수 있습니다.',howTitle:'MCP를 직접 구현하지 않고 Provider로 연결합니다.',howText:'RemoteMcpProvider가 외부 MCP의 연결과 tool 호출을 담당하고 ProviderRegistry가 이를 WSR의 MCP Server에 노출합니다. 새로운 MCP마다 별도의 Gateway를 만들 필요가 없습니다.',stepsTitle:'새 MCP를 연결하는 기본 흐름',stepsText:'실제 사용자는 연결 정보를 준비해 AI에게 작업을 요청하는 방식으로 확장할 수 있습니다. 개발자는 아래 구조만 이해하면 됩니다.',s1:'연결 정보를 확인',s1p:'MCP의 endpoint, transport, 인증 방식, tools/list 지원 여부와 실행 방법을 확인합니다.',s2:'Provider로 등록',s2p:'RemoteMcpProvider를 재사용해 id, namespace, URL을 등록합니다.',s3:'Namespace 적용',s3p:'원격 tool 이름 앞에 Provider namespace를 붙여 충돌을 방지합니다.',s4:'독립적으로 운영',s4p:'Provider가 꺼져도 WSR Core와 다른 기능은 계속 동작합니다.',exampleTitle:'이미 연결된',exampleText:'Godot MCP는 이 구조를 사용하는 실제 예입니다. 같은 패턴으로 Blender나 다른 Remote MCP도 연결할 수 있습니다.',lifeTitle:'Provider 하나가 꺼져도 Gateway는 계속됩니다.',lifeText:'ProviderScheduler가 상태를 확인하고 연결이 끊긴 Provider는 다시 연결을 시도합니다. Tool 목록이 바뀌면 Registry snapshot도 갱신합니다.',connected:'연결된 Provider health check',retry:'연결되지 않은 Provider 재시도',aiCalloutTitle:'MCP 서버를 켜고 AI에게 연결을 부탁하세요.',aiCalloutText:'WSR의 MCP 확장 구조와 등록 방법은 skills 문서로 정의되어 있습니다. 사용자는 MCP 서버를 실행한 뒤 AI에게 해당 MCP를 WSR에 연결해 달라고 요청하면 됩니다. 연결이 완료되면 WSR을 종료하고 start.bat를 다시 실행한 다음 ChatGPT 플러그인에서 액션 정보를 새로고침하세요.',aiTitle:'사람보다 AI가 연결 작업을 수행할 수 있도록.',aiText:'MCP 확장은 반복적인 설정 작업이 많은 영역입니다. WSR의 구조를 이해한 AI에게 원하는 MCP를 연결하도록 요청하는 방식도 자연스럽게 사용할 수 있습니다.'},en:{hero:'WSR is not tied to a single MCP. Remote MCP servers that support Streamable HTTP can be connected as Providers and safely extended through namespaces.',howTitle:'Connect MCPs through Providers, not custom gateways.',howText:'RemoteMcpProvider handles remote connections and tool calls, while ProviderRegistry exposes them through the WSR MCP server. You do not need a separate gateway implementation for every MCP.',stepsTitle:'The basic flow for adding an MCP',stepsText:'In practice, a user can provide the connection details and ask an AI to perform the integration. Developers only need to understand the structure below.',s1:'Inspect connection details',s1p:'Check the endpoint, transport, authentication, tools/list support and how the MCP server is started.',s2:'Register a Provider',s2p:'Reuse RemoteMcpProvider and register an id, namespace and URL.',s3:'Apply a namespace',s3p:'Prefix remote tool names with the Provider namespace to prevent collisions.',s4:'Keep it isolated',s4p:'The WSR Core and other capabilities continue working when one Provider is unavailable.',exampleTitle:'Already connected:',exampleText:'Godot MCP is a real example of this architecture. The same pattern can connect Blender or other Remote MCP servers.',lifeTitle:'The Gateway survives a Provider failure.',lifeText:'ProviderScheduler checks health and retries unavailable Providers. Registry snapshots are refreshed when tool definitions change.',connected:'Connected Provider health check',retry:'Unavailable Provider retry',aiCalloutTitle:'Start the MCP server and ask AI to connect it.',aiCalloutText:'The WSR MCP extension and registration process are defined in the skills documentation. Start the MCP server, then ask an AI to connect it to WSR. After the integration is complete, stop WSR, run start.bat again, and refresh the actions in the ChatGPT plugin.',aiTitle:'Let AI do more of the integration work.',aiText:'MCP extension often involves repetitive configuration. You can naturally ask an AI that understands the WSR structure to connect the MCP you need.'}}
